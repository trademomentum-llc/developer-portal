...
| G14 | Dependabot open alerts went 3 -> 7 on the pushed series: urllib <=4.9.0 HIGH (GHSA-hq3h-g68c-hp78, CVSS 7.5, credential leak via cross-origin redirects) via infinispan ^0.12.0 -> urllib ^3.23.0 (patched 4.9.1, major bump -> yarn resolution needed); new react-router mediums beyond the 3 documented in SECURITY.md; elliptic low | OPEN 2026-08-26 (`gh api .../dependabot/alerts`, two pages) | Disposition: yarn resolution urllib->4.9.1 + compat check, or document accepted risk; re-sync SECURITY.md accepted-risk list; re-issue provenance certificate on dependency change |
