...
| G4 | Cluster-plane agent cert pins stale at renewal; current certs expire 2026-09-24 | OPEN -- `repin-plane-agent-ca.sh --check` 2026-08-24: all three pins match live certs, no drift today | Re-pin before expiry; structural fix (real CA) deferred upstream |
| G5 | m2i-6 OpenBao dev-mode inmem loses secrets on restart | IN PROGRESS -- recurrence proven 4x in 4 days (2x today); Requirements doc `docs/specs/2026-08-24-OpenBao-Production-Storage-Requirements.md` written; Raft+PVC proposed | Design + Technical Specs, then implementation per triad |
