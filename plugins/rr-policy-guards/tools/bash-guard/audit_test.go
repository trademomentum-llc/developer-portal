// audit_test.go -- hash-chain tests for the bash-guard audit writer
...
func TestAudit_PrevHashChain(t *testing.T) {
	...
	t.Setenv("RR_BASH_GUARD_AUDIT_LOG", path)

	logAudit("allow", "", "Bash", "s1")
	logAudit("block", "unquoted expansion", "Bash", "s1")
