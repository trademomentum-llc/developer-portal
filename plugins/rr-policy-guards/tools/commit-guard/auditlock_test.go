...
func TestAudit_ConcurrentWriters(t *testing.T) {
	...
	t.Setenv("RR_COMMIT_GUARD_AUDIT_LOG", path)
	...
	for w := 0; w < writers; w++ {
		...
		go func(w int) {
			...
			for i := 0; i < appendsEach; i++ {
				err := appendAuditStrict(AuditRecord{
					Decision: DecisionAllow,
					Mode:     "scan-staged",
					Subject:  fmt.Sprintf("concurrency-probe w%d", w),
				})
				if err != nil {
	...
	wg.Wait()
	if failures > 0 {
		t.Fatalf("%d appends failed", failures)
	}
...
func TestAudit_ConcurrentProcesses(t *testing.T) {
	if n := os.Getenv("RR_AUDIT_WORKER_LINES"); n != "" {
		...
		for i := 0; i < count; i++ {
			if err := appendAuditStrict(AuditRecord{Decision: DecisionAllow, Mode: "scan-staged", Subject: "process-probe"}); err != nil {
	...
	for p := 0; p < procs; p++ {
		...
		go func(p int) {
			...
			cmd.Env = append(os.Environ(),
				"RR_COMMIT_GUARD_AUDIT_LOG="+path,
